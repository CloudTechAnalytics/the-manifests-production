...
import { PIPELINE_STAGES, pickPrimaryCurrency } from '@/shared/lib/utils/status';
import type {
  Quotation,
  Shipment,
  ShipmentStatus,
...
/** Everything useDashboardData fetches, minus the bits that are derived
 *  (primaryCurrency*) or hook-level concerns (loading, isAdmin) rather
 *  than part of the fetch itself. This is the shape the SWR fetcher
 *  below returns as one object, and the shape SWR caches by key. */
type DashboardFetchResult = Omit<
  DashboardData,
  'loading' | 'primaryCurrency' | 'primaryCollectedCurrency' | 'isAdmin'
>;
...
/**
 * The actual fetch — every query independent of the others, issued as
 * one concurrent batch (awaiting them in sequence cost a full network
 * round-trip each; on a high-latency link that serialised delay, not
 * query time, dominated how long the dashboard took to paint). Returns
 * one aggregate object rather than calling setters directly, so
 * useDashboardData below can hand it straight to SWR as cacheable data —
 * navigating away and back within SWR's cache window repaints instantly
 * from cache instead of re-running all of this from an empty state.
 */
async function fetchDashboardData(
  ...
  return {
    ...
    revenueCollectedByCurrency: collected,
    revenueCollectedThisMonth: collectedThisMonth,
    quotationConversionRate: conversionRate,
    shipmentCompletionRate: completionRate,
    pipeline: pipelineCounts,
    revenueTrend: trend,
    alerts: {
    ...
    recentShipments: (recentShips as Shipment[]) ?? [],
    recentQuotations: (recentQuots as Quotation[]) ?? [],
 ...
 *
 * Backed by SWR (keyed on profile id / role / branch) rather than plain
 * useState+useEffect: navigating away and back to the dashboard within
 * SWR's cache window repaints instantly from cache instead of blocking
 * on the full ~13-query fetch again from an empty state every time.
 */
export function useDashboardData(): DashboardData {
  ...
  const { data, isLoading } = useSWR(
    profile?.id ? ['dashboard-data', profile.id, isAdmin, branchFilter] : null,
    () => fetchDashboardData(isAdmin, branchFilter),
  ...
  return {
    loading: isLoading,
    stats: result.stats,
    revenueByCurrency: result.revenueByCurrency,
    primaryCurrency: pickPrimaryCurrency(result.revenueByCurrency),
    revenueCollectedByCurrency: result.revenueCollectedByCurrency,
    primaryCollectedCurrency: pickPrimaryCurrency(result.revenueCollectedByCurrency),
    revenueCollectedThisMonth: result.revenueCollectedThisMonth,
    quotationConversionRate: result.quotationConversionRate,
    shipmentCompletionRate: result.shipmentCompletionRate,
    pipeline: result.pipeline,
    revenueTrend: result.revenueTrend,
    alerts: result.alerts,
    recentShipments: result.recentShipments,
    recentQuotations: result.recentQuotations,
    recentDeliveries: result.recentDeliveries,
    topCustomersByVolume: result.topCustomersByVolume,

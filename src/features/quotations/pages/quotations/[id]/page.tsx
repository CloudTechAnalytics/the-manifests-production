...
import { computeApprovalReason, type QuotationTotals } from '@/shared/lib/quotation-schema';
import { resolveScopeOfServiceLabel } from '@/shared/lib/quotation-rules';
...
export default function QuotationDetailPage() {
        ...
        if (session) {
          ...
          try {
            ...
            const { buildQuotationPdfData, renderQuotationPdfBlob, blobToBase64 } = await import(
              '@/shared/lib/pdf/generate-quotation-pdf'
            );
          ...
          const emailResponse = await fetch(
            `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-quotation-status-email`,
            {
              method: 'POST',
              headers: {
  ...
  const buildPdfBlob = async () => {
    ...
    try {
      const { buildQuotationPdfData, renderQuotationPdfBlob } = await import('@/shared/lib/pdf/generate-quotation-pdf');
  ...
  const handleSendToCustomer = async () => {
    ...
    try {
      const blob = await buildPdfBlob();
      if (!blob) return;
      const { blobToBase64 } = await import('@/shared/lib/pdf/generate-quotation-pdf');

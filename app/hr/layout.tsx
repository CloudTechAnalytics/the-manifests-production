...
export default function HrLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // A platform_admin has no organization_id — HR Workspace isn't their
    // workspace at all, same reasoning as the main tenant shell bouncing
    // them to /platform.
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <aside className="relative hidden h-screen w-64 shrink-0 flex-col bg-sidebar lg:flex">
        <SidebarContent pathname={pathname} hasRole={hasRole} orgName={profile.organization?.name} />
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex-1 overflow-y-auto scrollbar-thin p-6 lg:p-8">
          {locked ? <FeatureLocked feature="HR & People Management" planName={planName} /> : children}
        </main>
      </div>
    </div>
  );
